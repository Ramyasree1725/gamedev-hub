...
"""
Notifications API routes for GameDev Hub.
Blueprint providing REST endpoints related to notifications.
"""
...
def _id() -> str:
    global _counter
    _counter += 1
    return f"notifications_{_counter:06d}_" + "".join(random.choices(string.ascii_lowercase, k=6))

@bp.route("/", methods=["GET"])
def list_items():
    """List all items for notifications."""
...
def create_item():
    """Create a new notifications resource."""
...
# Domain-specific helpers for notifications
class NotificationsService:
    """Service class encapsulating notifications business logic."""
...
def seed():
    for n in range(5):
        ...
        _store[item_id] = {
            "id": item_id,
            "name": f"Sample notifications {n+1}",
